...
void SvgWriter::drawBackground(svg::Document& doc)
{
  ...
  background << svg::Point(-MARGIN, -MARGIN)
             << svg::Point(layout_.gridW + MARGIN, -MARGIN)
             << svg::Point(layout_.gridW + MARGIN, layout_.gridH + MARGIN)
...
{
  doc << svg::Circle(
      svg::Point(0, 0), CORNER_VIA_RADIUS, svg::Fill(BLACK),
      svg::Stroke(CORNER_VIA_RADIUS, BLACK));
  doc << svg::Circle(
      svg::Point(layout_.gridW - 1, 0), CORNER_VIA_RADIUS, svg::Fill(BLACK),
      svg::Stroke(CORNER_VIA_RADIUS, BLACK));
  doc << svg::Circle(
      svg::Point(layout_.gridW - 1, layout_.gridH - 1), CORNER_VIA_RADIUS,
      svg::Fill(BLACK), svg::Stroke(CORNER_VIA_RADIUS, BLACK));
  doc << svg::Circle(
      svg::Point(0, layout_.gridH - 1), CORNER_VIA_RADIUS, svg::Fill(BLACK),
      svg::Stroke(CORNER_VIA_RADIUS, BLACK));
}
...
void SvgWriter::drawVias(svg::Document& doc)
{
  for (int y = 0; y < layout_.gridH; ++y) {
    for (int x = 0; x < layout_.gridW; ++x) {
      doc << svg::Circle(
          svg::Point(x, y), VIA_RADIUS, svg::Fill(BLACK),
          svg::Stroke(VIA_RADIUS, BLACK));
    }
  }
}
...
void SvgWriter::drawStripCuts(svg::Document& doc)
{
  for (auto& v : layout_.stripCutVec) {
    ...
    doc << svg::Rectangle(
        start, CUT_W, CUT_H, svg::Fill(BLACK), svg::Stroke(VIA_RADIUS, BLACK));
  }
}
...
{
  doc << svg::Text(
      svg::Point(0.0, -2.0), titleStr, BLACK, svg::Font(2.0, "Arial"));
}

void SvgWriter::drawTitleMirror(svg::Document& doc, const std::string& titleStr)
{
  doc << svg::Text(
      svg::Point(layout_.gridW - 1, -2.0), titleStr, BLACK,
      svg::Font(2.0, "Arial"));
}
